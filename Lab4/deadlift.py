...
while True:
    ...
    if 1:                       #Change to '0' to stop showing the angles from the accelerometer
        outputString += "#  ACCX %5.2f ACCY %5.2f  #  " % (ACCx, ACCy)

    
    # Orientation of the board is such that the wires are in the back

    if (ACCy < 0):
        outputString += " Tilt Left"
    if 0:#(AccYangle > 0):
        outputString += " Tilt Down"
    if (ACCx > 0):
        outputString += " Tilt Forward"
    if 0:#(AccXangle > 0):
        outputString += " Tilt Right"
    #outputString = AccYangle
    print(outputString)
